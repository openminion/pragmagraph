...
1. RC tags such as `v0.0.6rc1` publish to TestPyPI.
2. Manual `workflow_dispatch` with `target=testpypi` publishes the final
   version to TestPyPI from the final release branch.
3. Final tags such as `v0.0.6` publish to production PyPI.
4. GitHub Releases should use the bare version title such as `0.0.6`.
